...
function buildRunnableChain(reRankingRetriever) {
  ...
  const ragChain = RunnableSequence.from([
    {
        context:reRankingRetriever.pipe(formatDocs),
...
async function answerQuestion({ reRankRetriever, ragChain }, question) {
...
async function main() {
  ...
  const ragChain = buildRunnableChain(reRankRetriever);

  const pipeline = { reRankRetriever, ragChain };
